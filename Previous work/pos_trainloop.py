...
if __name__ == '__main__':
    ...
    manager = TrainingManager('pos', paths, ranks)
    ...
    for i in range(maxiter + 1):
        ...
        # Rank accuracies
        true_indices = np.arange(p1.shape[0])
        ...
        # Save plots and images every 10 iterations
        if i % 10 == 0 or i == 1:
            for rank in ranks:
            ...
            fig, (ax1, ax2, ax3) = Plotter.plot_iteration_with_loss_and_accuracy(
                p1, p2, diag_matrix_val, manager.losses, manager.rank_accuracies10, train_loss.item(), i, maxiter
            )
    ...
    # Save the video
    imageio.mimsave(os.path.join(manager.paths['movies'], f'{formatted_time}_training_pos.mp4'), manager.frames, fps=2.5)