...
# Assuming you have already defined these imports
...
def train(model, rank, datetime_str, model_params, shared_dict, barrier, resume_training=False):
    ...
    while True:
        ...
        if rank == 0:
            ...
            if val_loss < best_val_loss:
                ...
                # Save model as it has improved
                torch.save(model.state_dict(), f"models/model_best_{datetime_str}.pth")
                saved = True  # Mark as saved if improvement occurs

            # Save checkpoint
            Checkpoint.save_checkpoint(model, optimizer, epoch, train_loss, val_loss, val_losses, train_corrects, val_corrects, "models/checkpoint.pth")
            ...
            Checkpoint.log_training(
                ...
                saved=saved,  # Log whether the model was saved
                file_name=f"training_log_{datetime_str}.csv"
            )

            # Use tqdm.write only in rank 0
            tqdm.write(
                f'Epoch {epoch + 1}: '
                f'train_loss={train_loss / train_iterations:.4f}, '
                f'val_loss={val_loss / val_iterations:.4f}, '
                f'train_correct={avg_train_correct:.4f}, '
                f'val_correct={avg_val_correct:.4f}, '
                f'model_saved={saved}'  # Use the 'saved' flag
            )
